...
class Aircraft:
    ...
    def instance_create(self):
        for i in range(len(instances)):
            if i==0: instances[i] = Aircraft(6,0)
            if i==1: instances[i] = Aircraft(48,6)
            if i==2: instances[i] = Aircraft(6,128)
            if i==3: instances[i] = Aircraft(9,81)
            if i==4: instances[i] = Aircraft(16,24)
...
if __name__=='__main__':
    ...
    for i in range(len(instances)):
        ...
        if i==1: instances[i] = Boeing_747(10,53,59,48)
        if i==2: instances[i] = Boeing_747(3,12,57,49)
        if i==3: instances[i] = Boeing_747(11,64,67,60)
        if i==4: instances[i] = Boeing_747(12,85,50,62)
