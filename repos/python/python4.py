...
if __name__=='__main__':
    ...
    for i in range(len(instances)):
        if i==0: instances[i] = Aircraft(8,0)
        if i==1: instances[i] = Aircraft(3,5)
        if i==2: instances[i] = Aircraft(6,18)
        if i==3: instances[i] = Aircraft(9,27)
        if i==4: instances[i] = Aircraft(24,24)
